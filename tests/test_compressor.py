...
@pytest.mark.skipif((not torch.cuda.is_available()) or (not HAS_INT4_EXT), reason="CUDA and built int4_ext._C are required")
def test_compress_kv_cache_int4_roundtrip():
    ...
    restored = compressor.decompress((ck, cv), meta)
...
@pytest.mark.skipif((not torch.cuda.is_available()) or (not HAS_INT4_EXT), reason="CUDA and built int4_ext._C are required")
def test_batch_decompress_partial_mode_with_tuple_input():
    compressor = QuantizedCompressor(bits=4)
    ...
    metas = []
    for i in range(kv_blocks.size(0)):
        ck, cv, meta = compressor.compress(kv_blocks[i], mode="k")
        ck_list.append(ck)
        cv_list.append(cv)
        metas.append(meta)

    ck_batch = torch.stack(ck_list, dim=0)
    cv_batch = torch.stack(cv_list, dim=0)

    restored = compressor.batch_decompress((ck_batch, cv_batch), metas)

    assert restored.shape == kv_blocks.shape
    assert torch.allclose(restored[:, 0], kv_blocks[:, 0], atol=0.35, rtol=0.35)
    assert torch.equal(restored[:, 1], kv_blocks[:, 1])
